class Map {
    ...
    loadMap(mapData) {
        ...
        let mapProperties = mapData.mapProperties
        let physics = mapData.mapProperties.physics

        this.w = mapProperties.width;
        this.h = mapProperties.height;
        this.groundMap = mapData.groundMap;

        //create reusable tileset
        this.tileSet = {
            tileSize: mapProperties.tileSize,
            ground: this.createTileImages(mapProperties.tilePaths.ground),
            wall: this.createTileImages(mapProperties.tilePaths.wall)
        };

        //physics
        this.friction = physics.friction;
        this.gravity = physics.gravity;
        this.stopZone = physics.stopZone;
        this.grace = physics.grace;
        this.floor = mapProperties.floor;
        this.collideReflect = physics.collideReflect;
        
        //create map walls
        let blankImg = new Image();
        blankImg.src = "";
        let mapWallWidth = 10;
        let mapWallHeight = 100;
        /*
        //left
        this.blocks.push(new Block(
            -1,
            new Vect3(-mapWallWidth, 0, 0), //x y z
            new Vect3(mapWallWidth, this.h, mapWallHeight), // W L H
            blankImg));
        
        //right
        this.blocks.push(new Block(
            -1,
            new Vect3(this.w, 0, 0), //x y z
            new Vect3(mapWallWidth, this.h, mapWallHeight), // W L H
            blankImg));
        ...
        */
        ...
        for (let i = 0; i < mapData.walls.length; i++) {
            ...
            if (typeof wall.tileOverride != 'undefined') {
                if (wall.tileOverride.top) {
                    topOverride = new Image();
                    topOverride.src = wall.tileOverride.top;
                }

                if (wall.tileOverride.side) {
                    sideOverride = new Image();
                    sideOverride.src = wall.tileOverride.side
                }
            }

            let images = {
                img: typeof topOverride == 'undefined' ? tiles.top : topOverride,
                imgSide: typeof sideOverride == 'undefined' ? tiles.side : sideOverride
            }
    ...
    createTileImages(paths) {
        //I have to do this weirdness because I need to store copies of the paths dict and js automatically stores references
        let tilePaths = JSON.parse(JSON.stringify(paths));
        let tileSet = JSON.parse(JSON.stringify(paths));
